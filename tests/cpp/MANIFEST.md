...
| **Column kernel, B2 constructs**: the `ParallelFor` lambda of `bt_cont` — a per-cell flag `do_i(i,j,0) != 0` on an `Array4<const int>` (the integer array is admitted only in this shape: the read IS the Bool) bound to a `const bool` local (bool locals are `let`s of Bool-valued expressions; `bool_local_point` in `test_kernel_join_locals.cpp` pins the point-kernel form); `const Real` PROLOGUE locals of the enclosing function captured by the lambda and hoisted into the model; `if (active) { … } else { … }` around two `for k` folds (one with a conditional step, one with two state variables) and the tail; `else if` flattened into the branch list (the same IR as flang's `ElseIfBlock`); real literals printed from their SOURCE SPELLING (read at the JSON node's byte offset — clang's `value` is the long-double approximation, `0.1_rt` → `0.100000000000000000001`). REFUSALS: an integer Array4 read as a value (`!= 1`), a non-const function-scope Real captured by the lambda | `test_kernel_bt_cont.cpp` / `bt_cont`, `refuse_flag_value`, `refuse_nonconst_capture` | `clang_kernel.annotate_literal_spellings`/`load_function`/`_lit_text`, `_is_flag_array_read`, `_extract_array_read(as_flag)`, `_extract_column_param` (flag arrays), `_extract_vardecl` (bool locals), `extract_column_kernel_from_decl` (prologue locals), `_extract_if` (else-if flattening); pinned by `tests/test_column.py::TestBtContFixture` |
